...
def create_one_batch(x, word2id, char2id, config, oov='<oov>', pad='<pad>', sort=True):
    ...
    if char2id is not None:
        bow_id, eow_id, oov_id, pad_id = char2id.get('<eow>', None), char2id.get('<bow>', None), char2id.get(oov,
                                                                                                             None), char2id.get(
            pad, None)
...
def train():
    ...
    if valid_data is not None:
        valid = create_batches(
            valid_data, opt.batch_size, word_lexicon, char_lexicon, config, sort=False, shuffle=False,
            use_cuda=use_cuda)
    else:
        valid = None
