...
class DQN(nn.Module):
    ...
    #     self.head2 = nn.Linear(256, num_actions)
    #
    #     self.relu = nn.ReLU()
    #     self.lrelu = nn.LeakyReLU(0.01)
    #
...
class AgentDQN(Agent):
    ...
    def make_action(self, state, test=False):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                # print("####################tensor", self.online_net(torch.tensor(state)).cpu().numpy())
                # print("####################tensor",self.online_net(torch.tensor(state)).cpu().numpy().shape)
                action = self.online_net(torch.tensor(state)).max(1)[1].view(1, 1)
                # print("accccccccccccccccccccccccccccction",self.online_net(torch.tensor(state)).max(1)[1])
                return action.item()
    ...
    def update(self):
        ...
        # print("!!!!!batch.state",batch.state[0])
    ...
    def train(self):
        ...
        loss = 0
        average_rewardlist = [] #last n episodes
        steps_list = [] #number of time steps
        while(True):
            ...
            if episodes_done_num % self.display_freq == 0:
                ...
                average_rewardlist.append(total_reward / self.display_freq)  # last n episodes
                steps_list.append(self.steps)  # number of time steps
                total_reward = 0
